...
foreach ($period as $date_item) {
    ...
    if ($check_time_result && mysqli_num_rows($check_time_result) > 0) {
        $update_time_query = "
            UPDATE time
            SET time_off_id = $time_off_id
            WHERE user_id = $user_id AND tanggal = '$date_value'
        ";
        mysqli_query($koneksi, $update_time_query);
    } else {
        $insert_time_query = "
            INSERT INTO time (tanggal, user_id, time_login, geotagging, status, time_off_id)
            VALUES ('$date_value', $user_id, '00:00:00', '', 'On-time', $time_off_id)
        ";
